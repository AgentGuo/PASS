...
class TimeSeriesdata(object):
    ...
    def _get_cat_cols(self, cat_cov_cols):
        ...
        for col in cat_cov_cols:
            ...
            mapped = self.data_df[col].map(
                lambda x: dct[x]).to_numpy().transpose()  # pylint: disable=cell-var-from-loop
    ...
    def inverse_normalize(self, normalized_data):
        ...
        Args:
            normalized_data: numpy array or pandas DataFrame of normalized data

        Returns:
            original_data: numpy array or pandas DataFrame of original data
        """
        if not self.normalize:
            return normalized_data

        original_data = self.scaler.inverse_transform(normalized_data.transpose())
        return original_data.transpose()
